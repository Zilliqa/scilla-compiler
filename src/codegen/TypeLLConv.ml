...
module TypeDescr = struct
  ...
  let sprint_specl_dict specls =
    ...
    String.concat ~sep:" " (List.map specls.bystrspecl ~f:(fun i -> pp_typ (PrimType (Bystrx_typ i))))
  ...
  let update_specl_dict (specls : specl_dict) ty =
    ...
    if not (TypeUtilities.is_storable_type ty) then specls else
    match ty with
    | ADT (tname, tlist) ->
      let (non_this, this_and_rest) = List.split_while specls.adtspecl
        ~f:(fun (tname', _) -> tname <> tname')
      in
      ...
      | (_, this_specls) :: rest ->
        ...
        else { specls with adtspecl = (tname, tlist :: this_specls) :: (non_this @ rest) }
      | [] -> { specls with adtspecl = (tname, [tlist]) :: specls.adtspecl }
      )
    | MapType (kt, vt) ->
      if List.exists specls.mapspecl ~f:(fun (kt', vt') ->
        TypeUtilities.type_equiv kt kt' && TypeUtilities.type_equiv vt vt')
      then specls else { specls with mapspecl = (kt, vt) :: specls.mapspecl }
    | PrimType (Bystrx_typ x) ->
      if List.mem specls.bystrspecl x ~equal:(=)
      then specls else { specls with bystrspecl = x :: specls.bystrspecl }
    | _ -> specls
  ...
  let generate_typedescr llmod specls =
    let _ = DebugMessage.plog (sprintf "\nSpecialized types:\n%s\n\n" (sprint_specl_dict specls)) in
    let llctx = Llvm.module_context llmod in
    let i32_ty = Llvm.integer_type llctx 32 in
    ...
    let enum_typ = function
      ...
      | ADT _ -> pure 1
      | MapType _ -> pure 2
      | _ -> fail0 "GenLlvm: TypeDescr: enum_typ: internal error: unsupported type"
    in
    ...
    *)
    ...
    (* Int32 *)
    let primtydescr_int32 = Llvm.define_global (tempname "TyDescr_Int32_Prim") (Llvm.const_named_struct
      tydescr_prim_ty [|qi (enum_prims (Int_typ Bits32)); qi (enum_bitwidth Bits32)|]) llmod in
    let ty_int32 = PrimType (Int_typ Bits32) in
    let%bind tydescr_int32 = wrap_primty (tempname "TyDescr_Int32") ty_int32 primtydescr_int32 in
    Caml.Hashtbl.add tdescr ty_int32 tydescr_int32;
    (* Uint32 *)
    let primtydescr_uint32 = Llvm.define_global (tempname "TyDescr_Uint32_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Uint_typ Bits32)); qi (enum_bitwidth Bits32)|]) llmod in
    let ty_uint32 = (PrimType (Uint_typ Bits32)) in
    let%bind tydescr_uint32 = wrap_primty (tempname "TyDescr_Uint32") ty_uint32 primtydescr_uint32 in
    Caml.Hashtbl.add tdescr ty_uint32 tydescr_uint32;
    (* Int64 *)
    let primtydescr_int64 = Llvm.define_global (tempname "TyDescr_Int64_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Int_typ Bits64)); qi (enum_bitwidth Bits64)|]) llmod in
    let ty_int64 = PrimType (Int_typ Bits64) in
    let%bind tydescr_int64 = wrap_primty (tempname "TyDescr_Int64") ty_int64 primtydescr_int64 in
    Caml.Hashtbl.add tdescr ty_int64 tydescr_int64;
    (* Uint64 *)
    let primtydescr_uint64 = Llvm.define_global (tempname "TyDescr_Uint64_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Uint_typ Bits64)); qi (enum_bitwidth Bits64)|]) llmod in
    let ty_uint64 = (PrimType (Uint_typ Bits64)) in
    let%bind tydescr_uint64 = wrap_primty (tempname "TyDescr_Uint64") ty_uint64 primtydescr_uint64 in
    Caml.Hashtbl.add tdescr ty_uint64 tydescr_uint64;
    (* Int128 *)
    let primtydescr_int128 = Llvm.define_global (tempname "TyDescr_Int128_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Int_typ Bits128)); qi (enum_bitwidth Bits128)|]) llmod in
    let ty_int128 = PrimType (Int_typ Bits128) in
    let%bind tydescr_int128 = wrap_primty (tempname "TyDescr_Int128") ty_int128 primtydescr_int128 in
    Caml.Hashtbl.add tdescr ty_int128 tydescr_int128;
    (* Uint128 *)
    let primtydescr_uint128 = Llvm.define_global (tempname "TyDescr_Uint128_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Uint_typ Bits128)); qi (enum_bitwidth Bits128)|]) llmod in
    let ty_uint128 = (PrimType (Uint_typ Bits128)) in
    let%bind tydescr_uint128 = wrap_primty (tempname "TyDescr_Uint128") ty_uint128 primtydescr_uint128 in
    Caml.Hashtbl.add tdescr ty_uint128 tydescr_uint128;
    (* Int256 *)
    let primtydescr_int256 = Llvm.define_global (tempname "TyDescr_Int256_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Int_typ Bits256)); qi (enum_bitwidth Bits256)|]) llmod in
    let ty_int256 = PrimType (Int_typ Bits256) in
    let%bind tydescr_int256 = wrap_primty (tempname "TyDescr_Int256") ty_int256 primtydescr_int256 in
    Caml.Hashtbl.add tdescr ty_int256 tydescr_int256;
    (* Uint256 *)
    let primtydescr_uint256 = Llvm.define_global (tempname "TyDescr_Uint256_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims (Uint_typ Bits256)); qi (enum_bitwidth Bits256)|]) llmod in
    let ty_uint256 = (PrimType (Uint_typ Bits256)) in
    let%bind tydescr_uint256 = wrap_primty (tempname "TyDescr_Uint256") ty_uint256 primtydescr_uint256 in
    Caml.Hashtbl.add tdescr ty_uint256 tydescr_uint256;
    (* String *)
    let primtydescr_string = Llvm.define_global (tempname "TyDescr_String_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims String_typ); qi 0|]) llmod in
    let ty_string = PrimType String_typ in
    let%bind tydescr_string = wrap_primty (tempname "TyDescr_String") ty_string primtydescr_string in
    Caml.Hashtbl.add tdescr ty_string tydescr_string;
    (* Bystr *)
    let primtydescr_bystr = Llvm.define_global (tempname "TyDescr_Bystr_Prim") (Llvm.const_named_struct
      tydescr_prim_ty  [|qi (enum_prims Bystr_typ); qi 0|]) llmod in
    let ty_bystr = PrimType Bystr_typ in
    let%bind tydescr_bystr = wrap_primty (tempname "TyDescr_Bystr") ty_bystr primtydescr_bystr in
    Caml.Hashtbl.add tdescr ty_bystr tydescr_bystr;
    (* BystrX *)
    let%bind _ = iterM specls.bystrspecl ~f:(fun x ->
      let primtydescr_bystrx = Llvm.define_global (tempname (sprintf "TyDescr_Bystr%d_Prim" x))
        (Llvm.const_named_struct tydescr_prim_ty  [|qi (enum_prims (Bystrx_typ x)); qi x|]) llmod in
      let ty_bystrx = (PrimType (Bystrx_typ x)) in
      let%bind tydescr_bystrx = wrap_primty (tempname (sprintf "TyDescr_Bystr%d" x))
         ty_bystrx primtydescr_bystrx in
    ...
    *)
    ...
    Llvm.struct_set_body tydescr_specl_ty
      [|
        i32_ty                             (* m_numTArgs *);
        ptr_ptr_ty tydescr_ty;             (* Typ** m_TArgs *)
        ptr_ptr_ty tydescr_constr_ty;      (* Constr** m_constrs *)
        Llvm.pointer_type tydescr_adt_ty;  (* ADTType* m_parent *)
      |] false;
    ...
    let tydescr_map_ty = Llvm.named_struct_type llctx (tempname "TyDescr_MapTyp") in
    Llvm.struct_set_body tydescr_map_ty
      [|
        Llvm.pointer_type tydescr_ty; (* Typ* m_keyTyp *)
        Llvm.pointer_type tydescr_ty; (* Typ* m_valTyp *)
      |] false;
    (* Declare type descriptors for all Maps. *)
    let%bind _ = iterM specls.mapspecl ~f:(fun (kt, vt) ->
      let ty_map = MapType (kt, vt) in
      let tydescr_map = declare_unnamed_const_global tydescr_ty (tempname "TyDescr_Map") llmod in
    ...
    let%bind _ = iterM specls.adtspecl ~f:(fun (tname, specls) ->
      ...
      let%bind tydescr_adt_decl =
        ...
        pure (declare_unnamed_const_global tydescr_adt_ty tvname llmod) in
      let%bind tydescr_specls_specls = mapM specls ~f:(fun specl ->
        let ty_adt = ADT (tname, specl) in
        let%bind tydescr_constrs = mapM adt.tconstr ~f:(fun c ->
          let%bind argts = TypeUtilities.constr_pattern_arg_types ty_adt c.cname in
          let%bind argts_ll = mapM argts ~f:(fun t -> resolve_typdescr tdescr t) in
          let%bind argts_ll_array =
            let%bind tvname = (tempname_adt (tname ^ "_" ^ c.cname) specl "Constr_m_args") in
            pure @@ define_unnamed_const_global tvname
            (Llvm.const_array (Llvm.pointer_type tydescr_ty) (Array.of_list argts_ll)) llmod in
          let num_args = List.length argts in
          let%bind cname_val = define_adtname c.cname in
          let tydescr_constr = Llvm.const_named_struct tydescr_constr_ty
            [|
              cname_val;
              qi num_args;
        ...
        let%bind tydescr_constrs_array =
          let%bind tvname = (tempname_adt tname specl "ADTTyp_Specl_m_constrs") in
          pure @@ define_unnamed_const_global tvname (Llvm.const_array
            (Llvm.pointer_type tydescr_constr_ty) (Array.of_list tydescr_constrs)) llmod in
        let num_targs = List.length specl in
        let%bind tydescr_targs_ll = mapM specl ~f:(fun t -> resolve_typdescr tdescr t) in
        let%bind tydescr_targs_array =
          let%bind tvname = (tempname_adt tname specl "ADTTyp_Specl_m_TArgs") in
          pure @@ define_unnamed_const_global tvname
          (Llvm.const_array (Llvm.pointer_type tydescr_ty) (Array.of_list tydescr_targs_ll)) llmod in
        let tydescr_specl = Llvm.const_named_struct tydescr_specl_ty
          [|
            qi num_targs;
            Llvm.const_bitcast tydescr_targs_array (ptr_ptr_ty tydescr_ty);
            Llvm.const_bitcast tydescr_constrs_array (ptr_ptr_ty tydescr_constr_ty);
            tydescr_adt_decl;
          |]
        in
      ...
      let%bind tydescr_specls_array =
        ...
        pure @@ define_unnamed_const_global tvname (Llvm.const_array
          (Llvm.pointer_type tydescr_specl_ty) (Array.of_list tydescr_specl_ptrs)) llmod in
      ...
      let tydescr_adt = Llvm.const_named_struct tydescr_adt_ty
        [|
          tname_val;
          qi num_constrs;
          qi num_specls;
          Llvm.const_bitcast tydescr_specls_array (ptr_ptr_ty tydescr_specl_ty);
        |]
      in
  ...
  let gather_specls_ty specls ty =
    let rec go inscope specls ty =
      ...
      if List.mem inscope ty ~equal:TypeUtilities.type_equiv then specls else
      match ty with
  ...
  let rec gather_specls_stmts specls stmts =
    ...
    foldM stmts ~init:specls ~f:(fun specls (stmt, _) ->
      ...
      | MatchStmt (_, clauses, jopt) ->
        ...
        foldM clauses ~init:specls_jopt ~f:(fun specls (pat, body) ->
          let%bind specls_bounds =
            foldM (get_spattern_bounds pat) ~init:specls ~f:(fun specls v ->
      ...
      | CallProc _ | Throw _ | Ret _ | StoreEnv _ | AllocCloEnv _ -> pure specls
    )
  ...
  let generate_type_descr_cmod llmod topclos cmod =
    ...
    let%bind specls_clos = foldM topclos ~init:empty_specl_dict
        ~f:(fun specls clo -> gather_specls_clo specls clo)
    in
    (* Library statements *)
    let%bind specls_libs = gather_specls_stmts specls_clos cmod.lib_stmts in
    (* Contract parameters *)
    let specls_params =
      List.fold cmod.contr.cparams ~init:specls_libs
        ~f:(fun specls (_, pt) -> gather_specls_ty specls pt)
    in
    (* Fields *)
    let%bind specls_fields = foldM cmod.contr.cfields ~init:specls_params
    ...
    let%bind specls_comps = foldM cmod.contr.ccomps ~init:specls_fields
    ~f:(fun specls c ->
        let specls_comp_params =
          List.fold c.comp_params ~init:specls
          ~f:(fun specls (_, pt) -> gather_specls_ty specls pt)
        in
