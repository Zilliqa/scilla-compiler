...
module ScillaCG_FlattenPat = struct
  ...
  type ('a, 'b, 'rep) match_handlers = {
    match_extractor : 'b -> (eannot ident * (FPS.spattern * 'b) list * 'b join_t option) option * 'rep;
    match_constructor : (eannot ident * (FPS.spattern * 'b) list * 'b join_t option) * 'rep -> 'b;
  ...
  let match_handlers_expr = { 
    match_extractor =
      (fun (e, rep) -> match e with
      | FPS.MatchExpr (o, clauses, joinopt) -> Some (o, clauses, joinopt), rep
      | _ -> None, rep);
    match_constructor =
      (fun ((o, clauses, joinopt), rep) -> (FPS.MatchExpr(o, clauses, joinopt), rep));
    jump_constructor =
      (fun (label, rep) -> (FPS.JumpExpr label, rep));
  ...
  let match_handlers_stmt = {
    match_extractor =
      ...
      | [(FPS.MatchStmt (o, clauses, joinopt), rep)] -> Some (o, clauses, joinopt), rep
      | _ -> None, empty_annot);
    match_constructor =
      (fun ((o, clauses, joinopt), rep) -> [FPS.MatchStmt(o, clauses, joinopt), rep]);
    jump_constructor =
      (fun (label, rep) -> [FPS.JumpStmt label, rep]);
      ...
      | (first_pat, (_, first_clause_rhs)) as first_clause :: rest_clauses ->
        ...
        | curobj :: remobjs ->
          ...
          let%bind (joinopt, joinstack') =
            ...
            let rem_clauses_l = List.map rem_clauses ~f:(fun (p, (pl, rhs)) -> (p :: pl, rhs)) in
            let%bind join_block = simplifier joinstack obj_l rem_clauses_l in
          ...
          | Constructor _ -> (* Constructor Rule. *)
            ...
            let%bind spats_rhs = mapM now_grouped ~f:(fun cur_cn_group ->
              ...
              let%bind (spat, subobjs) = (match cur_cn_group with
                | [] -> fail0 "FlatternPatterns: Internal error: empty group of constructors."
                | (cur_cn, _) :: _ ->
                  ...
                  | Constructor (cname, cargs) ->
                    ...
                    pure (spat, subobjs)
                  | _ -> fail0 ("FlattenPatterns: Internal error: " ^
                    "Found non constructor pattern when handling Constructor rule.")
                  )
              ) in
              (* Compute the body of each branch of the new match. *)
              let%bind subclauses = mapM cur_cn_group ~f:(fun (p, (plist, e)) ->
                match p with
                | Constructor (_, cargs) -> pure ((cargs @ plist), e)
                | _ -> fail0 ("FlattenPatterns: Internal error: " ^
                  "Found non constructor pattern when handling constructor group.")
              ) in
          ...
          | Wildcard | Binder _ -> (* Variable Rule. *)
            let%bind clauses' = mapM now_clauses ~f:(fun (p, (plist, e)) ->
              match p with
                ...
                | Binder v ->
                  let bound_vars_in_rempats = List.fold plist ~init:[] ~f:(fun acc p ->
                    get_pattern_bounds p @ acc
                  ) in
                  if is_mem_id v bound_vars_in_rempats
                  then pure (plist, e)
            ...
            | None, _ -> pure simplified (* There is no possibility of a join. *)
            | Some _, Some (o, clauses, None) ->
              pure @@ handlers.match_constructor ((o, clauses, joinopt), r)
  ...
  let flatpat_in_expr newname (e, erep) =
    ...
    let rec go_expr (e, erep) = 
      ...
      | TFunMap texprl ->
        let%bind texprl' = mapM ~f:(fun (t, e) ->
      ...
      | MatchExpr (obj, clauses) ->
        ...
        match_simplifier go_expr newname match_handlers_expr erep obj_l clauses_l
    in
  ...
  let flatpat_in_module (cmod : cmodule) rlibs elibs =
    ...
    let contr' = { FPS.cname = cmod.contr.cname; FPS.cparams = cmod.contr.cparams;
                    FPS.cfields = fields'; ccomps = comps' } in
    let cmod' = { FPS.smver = cmod.smver; FPS.cname = cmod.cname;
                  FPS.elibs = cmod.elibs;
                  FPS.libs = clibs'; FPS.contr = contr' } in
