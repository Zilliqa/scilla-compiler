...
module ScillaCG_AnnotationExplicitizer
  ...
  let rec explicitize_stmts stmts =
    ...
    | (s, srep) :: sts ->
      ...
      | Store (m, i) ->
        ...
        pure ((s', srep_to_eannot srep) :: sts')
      | MapUpdate (i, il, io) ->
        let s' = EAS.MapUpdate (eid_to_eannot i, List.map eid_to_eannot il, BatOption.map eid_to_eannot io) in
        pure ((s', srep_to_eannot srep) :: sts')
      | MapGet (i, i', il, b) ->
        let s' = EAS.MapGet (eid_to_eannot i, eid_to_eannot i', List.map eid_to_eannot il, b) in
        pure ((s', srep_to_eannot srep) :: sts')
      | ReadFromBC (i, s) ->
  ...
  let explicitize_module (cmod : cmodule) rlibs elibs =
    ...
    let%bind rlibs' = explicitize_lib_entries rlibs
    in
    ...
    let%bind comps' = mapM ~f:(fun comp ->
      ...
      let comp_params' = List.map(fun (i, t) -> eid_to_eannot i, t) comp.comp_params in
      pure { 
    ...
    let contr' = 
      let params' = List.map(fun (i, t) -> eid_to_eannot i, t) cmod.contr.cparams in
      { EAS.cname = sid_to_eannot cmod.contr.cname;
        EAS.cparams = params';
        EAS.cfields = fields';
        ccomps = comps' } in
    let cmod' = 
      let eliblist = List.map (fun (a, b) -> sid_to_eannot a, BatOption.map sid_to_eannot b) cmod.elibs in
