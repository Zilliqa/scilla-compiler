  ...
  let translate_in_expr newname (e, erep) =
    ...
    let rec go_expr (e, erep) = 
        ...
        let rec uncurry_app (previous_temp : UCS.eannot ident) remaining =
          ...
          | [] ->
            let rep : Uncurried_Syntax.eannot =
              { ea_loc = erep.ea_loc; ea_tp = (get_rep previous_temp).ea_tp } in
          ...
          | next :: remaining' ->
            ...
            | Some (UCS.FunType (_, pt_ret)) ->
              let temp_rep : Uncurried_Syntax.eannot = 
                { ea_loc = (get_rep previous_temp).ea_loc; ea_tp = Some pt_ret }
              in
              let temp = newname (get_id a) temp_rep in
              let rep : Uncurried_Syntax.eannot =
                { ea_loc = erep.ea_loc; ea_tp = temp_rep.ea_tp } in
            ...
            | _ -> fail1 (sprintf "Uncurry: internal error: type mismatch applying %s."
                      (get_id a)) (get_rep a).ea_loc
            )
          )
        in
      ...
      | Let (i, topt, lhs, rhs) ->
        ...
        pure ((UCS.Let (translate_var i, Option.map ~f:translate_typ topt, lhs', rhs')), translate_eannot erep)
      | TFunMap texprl ->
      ...
      | MatchExpr (obj, clauses, joinopt) ->
        let%bind clauses' = mapM clauses ~f:(fun (p, rhs) ->
          let p' = translate_spattern p in
          let%bind rhs' = go_expr rhs in
          pure (p', rhs')
        ) in
        let%bind joinopt' = mapO joinopt ~f:(fun (l, je) ->
          let l' = translate_var l in
          let%bind je' = go_expr je in
          pure (l', je')
        ) in
        pure (UCS.MatchExpr (translate_var obj, clauses', joinopt'), translate_eannot erep)
      | JumpExpr l -> pure (UCS.JumpExpr (translate_var l), translate_eannot erep)
    ...
    let rec go_stmts stmts =
      foldrM stmts ~init:[] ~f:(fun acc (stmt, srep) ->
        ...
        | Throw t ->
          ...
          pure @@ (s', translate_eannot srep) :: acc
        | CallProc (p, al) ->
          let s' = UCS.CallProc(translate_var p, List.map ~f:translate_var al) in
          pure @@ (s', translate_eannot srep) :: acc
        | Bind (i , e) ->
        ...
        | MatchStmt  (obj, clauses, joinopt) ->
          ...
          pure @@ (UCS.MatchStmt (translate_var obj, clauses', joinopt'), translate_eannot srep) :: acc
        | JumpStmt j -> pure @@ (UCS.JumpStmt (translate_var j), translate_eannot srep) :: acc
        )
    ) in
    go_stmts stmts
  ...
  let translate_in_module (cmod : cmodule) rlibs elibs =
    ...
    let translate_in_lib_entries lentries =
      mapM ~f:(fun lentry ->
        ...
        | LibVar (i, topt, lexp) ->
          ...
          pure @@ (UCS.LibVar(translate_var i, Option.map ~f:translate_typ topt, lexp'))
        | LibTyp (i, ls) ->
    ...
    let%bind comps' = mapM ~f:(fun comp ->
      ...
      pure @@ {
        ...
        UCS.comp_params = List.map comp.comp_params 
              ~f:(fun (i, t) -> (translate_var i, translate_typ t));
        UCS.comp_body = body' 
      }
    ) cmod.contr.ccomps
    in

    let contr' = { UCS.cname = translate_var cmod.contr.cname; 
                    UCS.cparams = List.map cmod.contr.cparams
                      ~f:(fun (i, t) -> (translate_var i, translate_typ t));
                    UCS.cfields = fields'; ccomps = comps' } in
    let cmod' = { UCS.smver = cmod.smver;
                  UCS.cname = translate_var cmod.cname;
                  UCS.elibs = List.map cmod.elibs 
                    ~f:(fun (i, iopt) -> translate_var i, Option.map ~f:translate_var iopt);
                  UCS.libs = clibs'; UCS.contr = contr' } in
