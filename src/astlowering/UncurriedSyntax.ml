...
module Uncurried_Syntax = struct
  ...
  let rec pp_stmts (stmts: stmt_annot list) : string = 
    let s' = 
      List.fold_left stmts ~init:[] ~f:(fun acc_s stmt ->
        let s = 
          match fst stmt with 
          | Load (x1, x2) -> "(Load " ^ pp_eannot_ident x1 ^ " <- " ^ pp_eannot_ident x2 ^ ")"
          | RemoteLoad (x1, addr, x2) -> "(RemoteLoad " ^ pp_eannot_ident x1 ^ 
            "[" ^ pp_eannot_ident addr ^ "] <- " ^ pp_eannot_ident x2 
          | Store (x1, x2) -> "(Store " ^ pp_eannot_ident x1 ^ " := " ^ pp_eannot_ident x2 ^ ")"
          | Bind (x, e) -> "(Bind " ^ pp_eannot_ident x ^ " = " ^ pp_expr e ^ ")"
          | MapUpdate (m, kl, m_o) -> 
            let kl_s = 
              String.concat ~sep:" " 
          ...
          | MapGet (x, m, kl, _) ->
            let kl_s = 
              ...
              (List.map kl ~f:(fun i -> "[" ^ pp_eannot_ident i ^ "]"))
            in 
            "(MapGet " ^ pp_eannot_ident x ^ " <- " ^ pp_eannot_ident m ^ kl_s ^ ")"
          | RemoteMapGet (x, addr, m, kl, _) ->
            let kl_s = 
              String.concat ~sep:" " 
              (List.map kl ~f:(fun i -> "[" ^ pp_eannot_ident i ^ "]"))
            in 
            "(RemoteMapGet " ^ pp_eannot_ident addr ^ "." ^ pp_eannot_ident x 
            ^ " <- " ^ pp_eannot_ident m ^ kl_s ^ ")"
  ...
  let pp_component comp = 
    ...
    let comp_params_s =
      String.concat ~sep:", " (List.map comp.comp_params ~f:(fun (param,ty) -> 
        "(" ^ pp_eannot_ident param ^ ": " ^ pp_typ ty ^ ")"
      )) in 
    let comp_body_s = pp_stmts comp.comp_body in 
    comp_name_s ^ ": " ^ comp_type_s ^
    "(" ^ comp_params_s ^ ")" ^ "\n" ^ comp_body_s
      

  let pp_library l = 
    let name_s = pp_eannot_ident l.lname in 
    let lentries_s = String.concat ~sep:"\n" (List.map l.lentries ~f:(pp_lentry)) in
    "Library " ^ name_s ^ ": \n" ^ lentries_s

  let pp_contract cmod = 
    let name_s = pp_eannot_ident cmod.cname in 
    let cparams_s = String.concat ~sep:", " (List.map cmod.cparams ~f:(fun (p, ty) -> 
      "(" ^ pp_eannot_ident p ^ ": " ^ pp_typ ty ^ ")"
    )) in
    (* Note: we don't print the expression *)
    let cfields_s = String.concat ~sep:", " (List.map cmod.cfields ~f:(fun (p, ty, _) -> 
      "(" ^ pp_eannot_ident p ^ ": " ^ pp_typ ty ^ ")"
    )) in
    let ccomps_s = String.concat ~sep:"\n" (List.map cmod.ccomps ~f:(pp_component))
    in
    name_s ^ "(" ^ cparams_s ^ ")" ^ "\n" ^
    "Fields: " ^ cfields_s ^ "\n" ^ 
    "Components: " ^ ccomps_s ^ "\n"
